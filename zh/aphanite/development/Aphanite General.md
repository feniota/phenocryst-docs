...
这里定义了 Aphanite General API 的端点定义和实现细节，使用 TypeScript 类型描述 API 期待的请求体/返回的请求体。
...
- 客户端拿到 `otp_token`，一个一次性令牌，和密码具有同等效力，可以用来请求[登录 API](#login)。
...
要用 TOTP 登录，客户端应遵循上面的验证流程，将 `method` 设为 `totp`，并将当前时间点的 TOTP 私钥作为 OTP 验证码。
